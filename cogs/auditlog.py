...
class AuditLogCommands(commands.Cog, name = "Audit Log Commands"):
	...
	async def _auditlogs(self, ctx, limit = 15, user: discord.User = None):
		...
		for entry in AuditLogs:
			embed.add_field(name = str(entry.action).replace('_', ' ').replace('AuditLogAction.', '').capitalize(), value = f"Action Type: {str(entry.action.category).replace('_', ' ').replace('AuditLogActionCategory.', '').capitalize()}\nUser: {entry.user.mention}", inline = False)
