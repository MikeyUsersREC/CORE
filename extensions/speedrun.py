...
class Speedrun(commands.Cog, name = "Speedrun Commands"):
	...
	async def gamesearch(self, ctx, *, game=None):
		if game == None:
			...
			if not game == None:
				...
				embed.set_footer(text=f"{GameName} | CORE", icon_url=self.bot.user.avatar_url)
	...
	async def usersearch(self, ctx, user=None):
		if user == None:
			embed = discord.Embed(title="An error has occured.", description="An error has occured that has prevented the command to run properly.", color=core_color)
			await ctx.send(embed=embed)
		else:
	...
	async def categorysearch(ctx, game, *, category):
		...
		if category in CategoryDict:
			...
			embed.set_thumbnail(url=GameIcon)
			embed.set_footer(text=f"{GameObject.name} | CORE", icon_url=cccorege)
			await ctx.send(embed=embed)
		else:
	...
	async def leaderboardsearch(ctx, game, *, category):
		...
		for record in LeaderboardObject.runs:
			...
			if category in CategoryList:
				...
				embed.set_footer(text=f"Showing only 10 results | {Game.name}", icon_url=coremage)
